...
import { convertCsvJson } from "@/app/lib/devTools";
import CopyableOutput from "@/app/components/CopyableOutput";

export default function CsvJsonTool() {
  const [input, setInput] = useState("");
  const { result, direction, error } = useMemo(() => convertCsvJson(input), [input]);

  return (
    <div className="space-y-3">
      <div>
        <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1 block">
          Paste CSV or JSON — direction is detected automatically
        </label>
        <textarea
          value={input}
      ...
      {error ? (
        ...
        <CopyableOutput label={direction === "csv-to-json" ? "JSON" : "CSV"} value={result} />
      )}
    </div>
  );
}
